...
public class OrganizeService: BaseService<Organize, OrganizeOutputDto>, IOrganizeService
{
    ...
    {
        if (App.User?.UserInfo.UserType==UserTypeEnum.SuperAdmin)
            return new List<long>();

        YuebonCacheHelper yuebonCacheHelper=new YuebonCacheHelper();
        var userId = App.User.UserInfo.UserId;
        ...
        if (orgIdList == null || orgIdList.Count < 1)
        {
            ...
            // 当前所属机构
            if (!orgIdList.Contains(App.User?.UserInfo.CreateOrgId??0))
                orgIdList.Add(App.User?.UserInfo.CreateOrgId??0);
    ...
    private async Task<List<long>> GetUserOrgIdList(List<Role> roleList)
    {
        ...
        // 缓存当前用户最大角色数据范围
        yuebonCacheHelper.Add(CacheConst.KeyRoleMaxDataScope + App.User?.UserInfo.UserId, strongerDataScopeType);
    ...
    {
        var orgId = App.User?.UserInfo.CreateOrgId ?? 0;
